...
const NavBar = () => {
    return <nav role="navigation" className="flex h-16 justify-end items-center">
        ...
        <Link href={orgData.deticatedAction.url} className="px-5 py-2.5 relative rounded group overflow-hidden font-medium bg-sec-brand-600 inline-block ml-3">
            <span className="absolute top-0 left-0 flex w-full h-0 mb-0 transition-all duration-200 ease-out transform translate-y-0 bg-brand-600 group-hover:h-full opacity-90"></span>
            <span className="relative group-hover:text-white">{orgData.deticatedAction.label}</span>
        </Link>
    </nav>
};
